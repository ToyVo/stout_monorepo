// MultithreadedMatrixAddition.cpp : Defines the entry point for the console
// application.
//

// #include "stdafx.h"

// CS442_threads_MatrixAddition.cpp : Defines the entry point for the console
// application.
// By Dr. Alnaeli
// copyrights
...
vector<vector<unsigned long long int>>
    matrixD; // calculated in parallel using std::thread
// vector<vector<unsigned long long int>> matrixE; // calculated parallel using
// openMP
//-------------------------
...
int main() {
  ...
  int count = 0;
  for (int i = 0; i < Dim; i++)
    for (int j = 0; j < Dim; j++) { // just arbitrary numbers
...
void sequential_Matrix_Addition() {
  ...
  for (int draw = 0; draw < matrixD.size(); ++draw) {
    for (int dcolumn = 0; dcolumn < matrixD.size(); ++dcolumn) {
      ...
      matrixC[draw][dcolumn] =
          (matrixA[draw][dcolumn] + matrixB[draw][dcolumn]);
    }
  }
...
void print10X10(vector<vector<unsigned long long int>> vec) {
  ...
  // precondition: need to make sure that the vectors have at least 10X10 rows
  // and columns.
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
void constructMatrices() {
  // pre conditions: all matrices are declared as vector of vectors:
  // vector<vector<unsigned long long int>>
  for (int i = 0; i < Dim; i++) {
